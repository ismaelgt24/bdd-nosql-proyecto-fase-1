...
(async () => {
    ...
    const getData = async () => {
        ...
        const solictudes = await Promise.all([gameAPI.getPlataformas(),gameAPI.getEmpresas()]);
        ...
        for (let i = 0; i < empresas.length; i++) {
            ...
            try {
                ...
                let juegosLeidos = await gameAPI.getVideojuegosByEmpresa(empresa.id);
    ...
    for (const juego of videojuegos) {
        ...
        for (const plat of juego.ratings){//
            const aux = {
                id: plat.id,
                name: plat.name,
                slug: plat.slug,
